...
def test_import_images_across_nights_a():
    ...
    assert environment['db'].execute("SELECT COUNT(*) FROM fits_files").fetchone()[0] == 0, "Database should be empty before test"
...
def test_import_images_across_nights_b():
    ...
    testdata.create_test_fits(filename='test_image_3.fits', date=datetime(2023, 10, 1, 19, 0, 1))
    testdata.create_test_fits(filename='test_image_4.fits', date=datetime(2023, 10, 2, 19, 0, 1))
